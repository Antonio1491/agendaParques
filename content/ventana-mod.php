<?php session_start(); ?>
<!DOCTYPE html>
<html lang="es">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <meta http-equiv="X-UA-Compatible" content="ie=edge">
  <title>Agenda</title>
<?php
include "inc/head.php";

?>
<body>
<?php include "inc/encabezado.php"; ?>
	<div class="text-center">
    <h3>Contactos</h3>
	</div>
<!-- contenido de registros-->
<?php
$conectar=mysqli_connect("localhost","anprorgm_admin","Admin_*2016","anprorgm_registros");
//$conectar=mysqli_connect("localhost","root","", "anpr");
mysqli_set_charset($conectar,"utf8");
if(!$conectar){
  die ("Error en la conexión a la Base de Datos");
}
else{
			$sql= "SELECT b.nombre, a.contact_id, a.email, a.nombre, a.estado, a.municipio, a.colonia, a.direccion, a.cp
					FROM datos AS a
					LEFT JOIN categoria AS b ON b.categoria_id = a.categoria_id
            ORDER BY a.nombre ASC ";
            $num=1;
			$resultado=mysqli_query($conectar,$sql);

echo "
<section class='tabla'>
	<div class='contenedor'>
		<table class=''>
    	<thead>
	    <th >NOMBRE</th>
	    <!--<th>E-MAIL</th>-->
	    <th>ESTADO</th>
		<th>MUNICIPIO</th>
		<th>COLONIA</th>
		<th>DIRECCIÓN</th>
		<th id='cp'>CP</th>
		<th></th>
		<th></th>
    	</thead>
      ";
while($row=mysqli_fetch_array($resultado)){
  echo "<tr>
  <td>$num</td>
  <td id='nombre'>".$row[3]."</td>
  <!--<td class='email'><a href='mailto:".$row[2]."'>".$row[2]."</td>-->
  <td>".$row[4]."</td>

	<td>".$row[5]."</td>
	<td>".$row[6]."</td>
	<td>".$row[7]."</td>
	<td>".$row[8]."</td>
	<td><a href='detalle.php?folio=".$row['1']."' id='mas'> Ver más</a></td>
	<td><a href='inc/eliminar.php?folio=".$row['1']."' id='eliminar' class='eliminar'>Eliminar<a/></td></tr>";
$num++;
}
echo "
</table>
</div>
</section>";
}
?>
</div>
</body>
</html>
